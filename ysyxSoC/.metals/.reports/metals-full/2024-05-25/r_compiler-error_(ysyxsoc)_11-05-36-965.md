file://<WORKSPACE>/src/device/Keyboard.scala
...
action parameters:
offset: 1386
uri: file://<WORKSPACE>/src/device/Keyboard.scala
text:
```scala
package ysyx
...
class ps2Chisel extends Module {
  ...
  buffer := Mux(sampling, io.ps2.data ## buffer(8, 0), buffer)
  ...
  val fire = io.in.pready && io.in.pwrite && io.in.psel && @@

  ready := Mux(sampling && done, true.B, Mux(fire))
