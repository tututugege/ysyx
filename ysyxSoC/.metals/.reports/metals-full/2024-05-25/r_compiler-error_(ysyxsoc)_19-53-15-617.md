file://<WORKSPACE>/src/device/VGA.scala
### scala.ScalaReflectionException: value hValid is not a method
...
action parameters:
offset: 1379
uri: file://<WORKSPACE>/src/device/VGA.scala
text:
```scala
package ysyx
...
class vgaChisel extends Module {
  ...
  val hValid = (xCount > hActive.U) && @@

  vgaAddr := Mux(hValid, yCount - 36.U, 0.U)


  color  := ram.read(vgaAddr);

  io.vga.hsync := xCount > hFrontPorch.U
  io.vga.vsync := yCount > vFrontPorch.U

  io.vga.valid := (xCount > hActive.U) && (xCount <= hBackPorch.U) &&
    (yCount > vActive.U) && (yCount <= vBackPorch.U)
