...
### scala.ScalaReflectionException: value fire is not a method
...
class gpioChisel extends Module {
  ...
  val led    = RegInit("FFFF".U(16.W))
  val button = RegInit("FFFF".U(16.W))
  val seg    = RegInit(Vec(8, "FF".U(8.W)))

  val ledAddr    = io.in.paddr(3, 2) === "b00".U
  val buttonAddr = io.in.paddr(3, 2) === "b01".U
  val segAddr    = io.in.paddr(3, 2) === "b10".U

  val ledRdata    = 0.U(16.W) ## led
  val buttonRdata = 0.U(16.W) ## button
  val segRdata    = seg.asUInt
  val fire = io.in.pready && io.in.psel && @@
  ...
  io.in.prdata := MuxCase(
    0.U,
    Seq(
      (~io.in.pwrite && ledAddr) -> ledRdata,
      (~io.in.pwrite && buttonAddr) -> buttonRdata,
      (~io.in.pwrite && segAddr) -> segRdata
    )
  )
