file://<WORKSPACE>/src/main/3.EX/Divider.scala
### scala.ScalaReflectionException: value addOut is not a method
...
action parameters:
offset: 1916
uri: file://<WORKSPACE>/src/main/3.EX/Divider.scala
text:
```scala
import chisel3._
...
class Srt2Divider extends Module {
  ...
  val init :: idle :: quot :: fix :: done = Enum(5)

  val state = RegInit(idle)

  val xAbs    = RegEnable(Fill(32, io.signed && io.x(31)) ^ io.x + 1.U, state === idle && io.div)
  val yAbs    = RegEnable(Fill(32, io.signed && io.y(31)) ^ io.y + 1.U, state === idle && io.div)
  ...
  val divisor   = RegEnable(yAbs << lzdShamt, state === idle)
  val count     = Reg(UInt(6.W))

  // shift
  val lzd = Module(new Lzd(false))
  lzd.io.in := Mux(state === idle, yAbs, remainder(63, 32) ^ Fill(32, remainder(64)))

  val shsrc = Wire(UInt(32.W))
  ...
  shsrc := Mux(state === idle, xAbs, remainder)
...
class Lzd(useDecoder: Boolean) extends Module {
  ...
  if (useDecoder) {
    ...
    outVec(1) := z(7) && z(6) && (~z(4) || z(5) && z(3) && z(2))
