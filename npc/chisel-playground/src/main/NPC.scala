...
class NPC(XLEN: Int) extends Module {
  ...
  if (CONFIG_CACHE) {
    val instCache = Module(new Icache(1, 2, 4))
    instCache.io.in.r <> instAxiLite.r
    instCache.io.in.ar <> instAxiLite.ar
    arbiter.io.InstAxiLite.r <> instCache.io.out.r
    arbiter.io.InstAxiLite.ar <> instCache.io.out.ar
    arbiter.io.InstAxiLite.w <> instAxiLite.w
    arbiter.io.InstAxiLite.aw <> instAxiLite.aw
    arbiter.io.InstAxiLite.b <> instAxiLite.b
    when(commit.halt) {

      printf("\n")
      printf("************ cache ***************************\n")
      printf("cache access number           : %d\n", instCache.cacheAccess)
      printf("cahce miss number             : %d\n", instCache.cacheMiss)
      printf("\n")
    }
  ...
  /* performance counter */
  if (CONFIG_CNT) {
    val instArDelay  = RegInit(0.U(64.W))
    val instRDelay   = RegInit(0.U(64.W))
    val loadArDelay  = RegInit(0.U(64.W))
    val loadRDelay   = RegInit(0.U(64.W))
    val storeAwDelay = RegInit(0.U(64.W))
    val storeWDelay  = RegInit(0.U(64.W))
    val storeBDelay  = RegInit(0.U(64.W))

    val instFetchCnt = RegInit(0.U(64.W))
    val brCancelCnt  = Wire(UInt(64.W))

    val cycleCnt  = RegInit(0.U(64.W))
    val commitCnt = RegInit(0.U(64.W))
    val brCnt     = RegInit(0.U(64.W))
    val brTkCnt   = RegInit(0.U(64.W))
    val calCnt    = RegInit(0.U(64.W))
    val csrCnt    = RegInit(0.U(64.W))
    val ldCnt     = RegInit(0.U(64.W))
    val stCnt     = RegInit(0.U(64.W))
    ...
    commitCnt := Mux(commit.commit, commitCnt + 1.U, commitCnt)
    ...
    val preBrCancelCnt = RegInit(0.U(64.W))
    val IFBrCancelCnt  = RegInit(0.U(64.W))
    val IDBrCancelCnt  = RegInit(0.U(64.W))
    ...
    when(commit.halt) {
      ...
      printf("instruction commit number    : %d\n\n", commitCnt)
