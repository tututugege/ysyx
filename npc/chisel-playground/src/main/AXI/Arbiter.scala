...
class AxiLiteArbiter extends Module {
  ...
  val assert   = Reg(Bool())
  val assertId = Reg(Bool())

  assert   := Mux(ar.valid && ~ar.ready, true.B, false.B)
  assertId := Mux(ar.valid, ar.bits.arid, assertId)
  val masterAr = Mux(
    assert,
    Mux(assertId, io.DataAxiLite.ar.bits, io.InstAxiLite.ar.bits),
    Mux(
  ...
  io.InstAxiLite.ar.ready := ar.ready && (~io.DataAxiLite.ar.valid || assert && ~assertId)
  io.DataAxiLite.ar.ready := ar.ready && (assert && assertId || ~assert)
