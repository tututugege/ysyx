 ...
 * __on_rt_assert_hook 2022-06-04     Meco Man     remove strnlen 2022-08-24
 * Yunjie       make rt_memset word-independent to adapt to ti c28x (16bit word)
...
rt_weak const char *rt_hw_cpu_arch(void) { return "unknown"; }
 ...
 *
 * @param  s is the address of source memory, point to the memory block to be
 * filled.
 *
 * @param  c is the value to be set. The value is passed in int form, but the
 * function uses the unsigned character form of the value when filling the
 * memory block.
 *
...
#define TOO_SMALL(LEN) ((LEN) < LBLOCKSIZE)
  ...
  unsigned char d =
      (unsigned int)c & (unsigned char)(-1); /* To avoid sign extension, copy C
               to an unsigned variable. (unsigned)((char)(-1))=0xFF for 8bit and
               =0xFFFF for 16bit: word independent */
...
/**
 * @brief  This function will copy memory content from source address to
 * destination address.
 *
 * @param  dst is the address of destination memory, points to the copied
 * content.
 *
 * @param  src  is the address of source memory, pointing to the data source to
 * be copied.
 *
...
/**
 * @brief  This function will move memory content from source address to
 * destination address. If the destination memory does not overlap with the
 * source memory, the function is the same as memcpy().
 *
 * @param  dest is the address of destination memory, points to the copied
 * content.
 *
 * @param  src is the address of source memory, point to the data source to be
 * copied.
 *
...
/**
 * @brief  This function will return the first occurrence of a string, without
 * the terminator '\0'.
 *
...
/**
 * @brief  This function will compare two strings while ignoring differences in
 * case
 *
...
/**
 * @brief  This function will return the length of a string, which terminate
 * will null character.
 *
...
#endif
  rt_kprintf(" / | \\     %d.%d.%d build %s %s\n", (rt_int32_t)RT_VERSION_MAJOR,
             (rt_int32_t)RT_VERSION_MINOR, (rt_int32_t)RT_VERSION_PATCH,
             __DATE__, __TIME__);
 ...
 */
rt_weak int rt_vsnprintf(char *buf, rt_size_t size, const char *fmt,
                         va_list args) {
#ifdef RT_KPRINTF_USING_LONGLONG
...
#ifdef RT_PRINTF_PRECISION
      str = print_number(str, end, (unsigned long)va_arg(args, void *), 16,
                         qualifier, field_width, precision, flags);
#else
      str = print_number(str, end, (unsigned long)va_arg(args, void *), 16,
                         qualifier, field_width, flags);
#endif
      continue;
...
#ifdef RT_PRINTF_PRECISION
    str = print_number(str, end, num, base, qualifier, field_width, precision,
                       flags);
#else
...
rt_device_t rt_console_set_device(const char *name) {
  ...
  if (new_iodev != RT_NULL) {
    if (_console_device != RT_NULL) {
      ...
      rt_device_open(_console_device,
...
rt_inline void _smem_info(rt_size_t *total, rt_size_t *used,
                          rt_size_t *max_used) {
  if (total)
...
rt_inline void _slab_info(rt_size_t *total, rt_size_t *used,
                          rt_size_t *max_used) {
  if (total)
    *total = system_heap->total;
  if (used)
 ...
 */
rt_weak void rt_memory_info(rt_size_t *total, rt_size_t *used,
                            rt_size_t *max_used) {
  rt_base_t level;
...
/**
 * @brief  This function allocates a memory block, which address is aligned to
 * the specified alignment size.
 *
...
/**
 * @brief This function finds the first bit set (beginning with the least
