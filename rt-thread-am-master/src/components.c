...
/**
 * @brief  The system main thread. In this thread will call the
 * rt_components_init() for initialization of RT-Thread Components and call the
 * user's programming entry main().
 *
...
void rt_application_init(void) {
  ...
  tid = &main_thread;
  result =
      rt_thread_init(tid, "main", main_thread_entry, RT_NULL, main_thread_stack,
...
/**
 * @brief  This function will call all levels of initialization functions to
 * complete the initialization of the system, and finally start the scheduler.
 *
